...
- **Route by phase, not habit.** Use a frontier model and higher effort for architecture, planning, hard debugging, risky migrations, and final review. Once the plan is clear and the work is bounded, move routine implementation, tests, docs, and mechanical cleanup to a cost-efficient model such as Cursor's Composer, or to OSS / open-weight coding models such as Kimi or GLM when your toolchain supports them. Escalate back to the frontier lane when the cheaper lane gets stuck, changes architecture, or produces low-confidence evidence.
- **Compare model plus harness, not model name alone.** The same model can behave differently in an IDE, CLI, cloud runner, or custom agent harness. A cheaper model in a strong harness with good repo context can be the better day-to-day choice than a frontier model used loosely, while the frontier model still earns its cost on judgment-heavy work.
- **Match the model to the job.** Use a strong model for planning, architecture, and review; use a cheaper or faster one for mechanical work. Even when the price gap is only 1.5x to 2x, repeated routine turns add up. See the [model picks table](./06-resources.md#model-picks-per-task). Examples: Opus to plan, Sonnet or Cursor's Composer to implement; GPT-5.5 xhigh to plan, GPT-5.4 medium to build; or a frontier model to architect and an OSS / open-weight model to execute well-scoped edits.
...
> **Takeaway:** spend frontier reasoning where judgment is needed; route bounded execution to cheaper models and settings.
